...
export const getPlaylistById = async (req: Request, res: Response) => {
  try {
    ...
    if (isNaN(numericId)) {
      return res.status(400).json({ error: 'Geçersiz çalma listesi ID formatı' });
    }
    ...
    if (!playlist.isPublic && playlist.owner.id !== userId) {
      return res.status(403).json({ error: 'Bu çalma listesine erişim izniniz yok' });
    }
...
export const updatePlaylist = async (req: Request, res: Response) => {
  try {
    ...
    if (playlist.ownerId !== userId) {
      return res.status(403).json({ error: 'Bu çalma listesini değiştirme izniniz yok' });
    }
...
export const deletePlaylist = async (req: Request, res: Response) => {
  try {
    ...
    if (playlist.ownerId !== userId) {
      return res.status(403).json({ error: 'Bu çalma listesini silme izniniz yok' });
    }
...
export const addSongToPlaylist = async (req: Request, res: Response) => {
  try {
    ...
    if (playlist.ownerId !== userId) {
      return res.status(403).json({ error: 'Bu çalma listesine şarkı ekleme izniniz yok' });
    }
...
export const removeSongFromPlaylist = async (req: Request, res: Response) => {
  try {
    ...
    if (playlist.ownerId !== userId) {
      return res.status(403).json({ error: 'Bu çalma listesinden şarkı çıkarma izniniz yok' });
    }
    ...
    if (!playlistSong) {
      return res.status(404).json({ error: 'Bu şarkı çalma listesinde bulunamadı' });
    }
